...
class TestWebPatchMethod():

    @pytest.mark.patch
    @pytest.mark.patch_web
    @pytest.mark.parametrize("url_page, index", [("https://reqres.in/", "users/2")])
    def test_patch_UPDATE_Web(self, browser, url_page, index):
        
         
            browser.get(url_page)
            browser.implicitly_wait(5)
            
            button1 = browser.find_element(By.CSS_SELECTOR, '[data-id="patch"]')
